...
refreshIcons();
function centeredDeleteConfirm(message, title='确认删除'){
    return new Promise(resolve => {
        const dialog = document.createElement('dialog');
        dialog.className = 'viewport-delete-confirm';
        dialog.innerHTML = `<form method="dialog"><div class="viewport-delete-confirm-mark">!</div><div><strong>${escapeHtml(title)}</strong><p>${escapeHtml(message)}</p></div><footer><button value="cancel">取消</button><button class="danger" value="confirm">确认删除</button></footer></form>`;
...
function canvasOriginalMediaUrl(url){
    return StudioMedia.originalMediaUrl(url);
}
function canvasFileNameFromUrl(url=''){
    return StudioMedia.mediaFileName(url);
}
function canvasProxiedMediaUrl(url, name=''){
    return StudioMedia.proxiedMediaUrl(url, name);
}
function canvasDisplayMediaUrl(url, name=''){
    return StudioMedia.displayMediaUrl(url, name);
}
...
const expiredAssetUrls = new Set();
...
function chatApiProviders(){
    ...
    if(window.SaaSCanvasGateway?.active) return providers;
...
function imageModelOptions(selectedModel, providerId){
    ...
    const modelLabel = model => {
        const [logicalModel, outputSpec = ''] = String(model || '').split('|||');
        return outputSpec ? `${logicalModel} · ${outputSpec}` : logicalModel;
    };
    const options = models.map(model => `<option value="${escapeHtml(model)}" ${model === selectedValue ? 'selected' : ''}>${escapeHtml(modelLabel(model))}</option>`).join('');
    const hasSelected = models.includes(selectedValue);
    return `${hasSelected || !selectedValue ? '' : `<option value="${escapeHtml(selectedValue)}" selected>${escapeHtml(modelLabel(selectedValue))}</option>`}${options}`;
}
...
function serializableCanvasNode(node){
    const copy = {...(node || {})};
    if(copy.unavailable) return copy;
    delete copy.running;
...
async function saveCanvas(){
    ...
    try {
        ...
        const idleWaiters = canvasSaveIdleWaiters;
...
function validCustomPixelSize(width, height){
...
function validateCustomPixelSizeFor(node, resolutionKey='resolution', widthKey='customWidth', heightKey='customHeight'){
    if(node?.[resolutionKey] !== 'custom') return true;
    if(validCustomPixelSize(node[widthKey], node[heightKey])) return true;
    showErrorModal('自定义像素尺寸不对，请修改！', '尺寸提示');
    return false;
}

function waitForCanvasSaveIdle(timeoutMs=15000){
    if(!savingCanvasNow) return Promise.resolve();
    return new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
            if(settled) return;
            settled = true;
            const index = canvasSaveIdleWaiters.indexOf(finish);
            if(index >= 0) canvasSaveIdleWaiters.splice(index, 1);
            reject(new Error('canvas save timeout'));
        }, timeoutMs);
        canvasSaveIdleWaiters.push(finish);
    });
}
...
async function flushCanvasSave(){
    ...
    if(savingCanvasNow){
        saveCanvasAgain = true;
        await waitForCanvasSaveIdle();
    }
    clearTimeout(saveTimer);
    saveTimer = null;
    localCanvasDirty = true;
    if(!await saveCanvas()) throw new Error('canvas save failed');
}
...
function openSmartCanvasPage(id){
    if(!id) return;
    const editorUrl = window.SaaSCanvasGateway?.editorUrl?.(id, 'smart');
    window.location.href = editorUrl || `/workspace/canvases/${encodeURIComponent(id)}/smart?id=${encodeURIComponent(id)}`;
}
...
async function openCanvas(id){
    ...
    try {
        ...
        canvas.logs = canvas.logs || [];
        const prunedExpiredLogs = pruneGenerationLogs();
        nodes = canvas.nodes || [];
        connections = canvas.connections || [];
        viewport = localViewportForCanvas(canvas.id, canvas.viewport || {x:0, y:0, scale:1});
        canvas.viewport = {...viewport};
        ...
        if(prunedExpiredLogs) scheduleSave();
        if(!window.SaaSCanvasGateway?.active) startCanvasRemotePolling();
        setStatus('Ready');
    } catch(e) {
...
function applyRemoteCanvasData(remote){
    ...
    try {
        ...
        canvas = remote;
        canvas.logs = canvas.logs || [];
        prunedExpiredLogs = pruneGenerationLogs();
        nodes = canvas.nodes || [];
        connections = canvas.connections || [];
        viewport = localViewport;
        canvas.viewport = {...viewport};
...
function resetTransientRunState(list=nodes){
    (list || []).forEach(node => {
        if(!node || node.unavailable) return;
...
function canvasLocalAssetUrls(){
    ...
    const add = value => {
        ...
        if(url && (url.startsWith('/output/') || url.startsWith('/assets/') || url.startsWith('/api/media/'))) urls.add(url);
    };
    nodes.forEach(node => {
...
async function refreshMissingCanvasAssets(){
    ...
    expiredAssetUrls.clear();
    const urls = canvasLocalAssetUrls();
    if(!urls.length) return;
    try {
        const availability = await StudioMedia.checkMediaAvailability(urls);
        availability.missing.forEach(url => missingAssetUrls.add(url));
        availability.expired.forEach(url => {
            if(missingAssetUrls.has(url)) expiredAssetUrls.add(url);
        });
    } catch(e) {
...
async function requestDeleteCanvas(id, event){
    event?.preventDefault();
    event?.stopPropagation();
    closeCanvasMetaPopover();
    if(!await centeredDeleteConfirm(tr('canvas.moveToTrashConfirm'))) return;
    await deleteCanvas(id);
}
async function requestPurgeCanvas(id, event){
    event?.preventDefault();
    event?.stopPropagation();
    closeCanvasMetaPopover();
    if(!await centeredDeleteConfirm(tr('canvas.purgeConfirm'))) return;
...
async function requestCanvasDeletion(url){
    let res = await fetch(url, {method:'DELETE'});
    if(res.status !== 409) return res;
    const data = await res.json().catch(() => ({}));
    const detail = data?.detail || {};
    if(!detail.confirm_required) return res;
    if(!await centeredDeleteConfirm(detail.message || '画布仍有生成任务运行，确认删除？')) return null;
    const separator = url.includes('?') ? '&' : '?';
    return fetch(`${url}${separator}confirm_running_tasks=true`, {method:'DELETE'});
}
async function deleteCanvas(id, event){
    ...
    try {
        const res = await requestCanvasDeletion(`/api/canvases/${id}`);
...
async function purgeCanvas(id, event){
    ...
    setStatus('Deleting...');
    try {
        const res = await requestCanvasDeletion(`/api/canvases/${id}/purge`);
        if(!res){ setStatus(''); return; }
...
function canvasListUrlForProject(projectId){
    ...
    return window.SaaSCanvasGateway?.canvasListUrl || '/workspace/canvases';
}
            ...
            <div class="gen-settings">
                ...
                <div class="gen-settings-row ms-custom-size-row" style="display:none">
                    <label class="field">
                        ...
                        <input class="setting-input ms-custom-w-input" type="number" min="256" max="8192" step="16" value="${escapeHtml(node.msCustomWidth || '')}" placeholder="1024">
                    </label>
                    <label class="field">
                        <div class="setting-title">${tr('canvas.height')}</div>
                        <input class="setting-input ms-custom-h-input" type="number" min="256" max="8192" step="16" value="${escapeHtml(node.msCustomHeight || '')}" placeholder="1024">
                    </label>
                    ...
                    <div class="custom-size-guidance">宽高必须是 16 的倍数，范围 256–8192 像素</div>
                </div>
                ${msLoras.length ? `
                <div class="gen-settings-row">
...
function openImageNodeMenu(nodeId, clientX, clientY){
    ...
    const canKeep = isRetainableCanvasMedia(node);
    imageNodeMenu.innerHTML = `
        ...
        ${canKeep ? `<button class="menu-btn" data-image-keep="${escapeAttr(nodeId)}"><i data-lucide="bookmark-plus" class="w-4 h-4"></i><span>保留到画布</span></button>` : ''}
    ...
    const keepBtn = imageNodeMenu.querySelector('[data-image-keep]');
    if(keepBtn){
        keepBtn.onclick = async e => {
            e.stopPropagation();
            closeImageNodeMenu();
            await keepImageNodeMedia(nodeId);
        };
    }
...
function openOutputNodeMenu(nodeId, clientX, clientY){
    ...
    const keepableCount = (node.images || []).filter(isRetainableCanvasMedia).length;
    ...
    imageNodeMenu.innerHTML = `
        ...
        <button class="menu-btn" data-output-keep="${escapeAttr(nodeId)}" ${keepableCount ? '' : 'disabled'}><i data-lucide="bookmark-plus" class="w-4 h-4"></i><span>保留全部到画布</span></button>
    ...
    const keepBtn = imageNodeMenu.querySelector('[data-output-keep]');
    if(keepBtn){
        keepBtn.onclick = async e => {
            e.stopPropagation();
            closeImageNodeMenu();
            await keepOutputNodeMedia(nodeId);
        };
    }
...
function isRetainableCanvasMedia(item){
    if(!item || item.mediaState === 'persistent') return false;
    if(window.SaaSCanvasGateway?.active) return Boolean(item.media_id);
    return String(outputUrlValue(item) || '').startsWith('/api/media/');
}
async function promoteCanvasMedia(item){
    if(window.SaaSCanvasGateway?.active && item?.media_id){
        const response = await fetch(`/api/v1/media/${encodeURIComponent(item.media_id)}/retain-to-canvas`, {method:'POST'});
        if(!response.ok) throw new Error(await response.text() || '媒体当前无法保留');
        return {...await response.json(), url:outputUrlValue(item)};
    }
    return StudioMedia.promoteMedia(item, {ownerId:String(canvas?.id || '')});
}
async function keepImageNodeMedia(nodeId){
    const node = nodes.find(item => item.id === nodeId);
    if(!node?.url) return;
    try {
        const media = await promoteCanvasMedia(node);
        node.url = media.url || node.url;
        node.mediaState = media.state || 'persistent';
        render();
        scheduleSave();
        setStatus('已保留到画布');
    } catch(error) {
        showErrorModal(error.message || '保留媒体失败', '保留到画布失败');
    }
}
async function keepOutputNodeMedia(nodeId){
    const node = nodes.find(item => item.id === nodeId);
    if(!node) return;
    try {
        const nextItems = [];
        for(const item of (node.images || [])){
            const currentUrl = outputUrlValue(item);
            if(!isRetainableCanvasMedia(item)){
                nextItems.push(item);
                continue;
            }
            const media = await promoteCanvasMedia(item);
            nextItems.push(typeof item === 'string'
                ? {url:media.url || currentUrl, mediaState:media.state || 'persistent'}
                : {...item, url:media.url || currentUrl, mediaState:media.state || 'persistent'});
        }
        node.images = nextItems;
        render();
        scheduleSave();
        setStatus('已保留全部输出到画布');
    } catch(error) {
        showErrorModal(error.message || '保留媒体失败', '保留到画布失败');
    }
}
...
function outputDownloadableImageUrls(node){
    return (node?.images || []).map(outputUrlValue).filter(url => url && !isMissingAssetUrl(url) && (url.startsWith('/output/') || url.startsWith('/assets/') || url.startsWith('/api/media/')));
}
...
async function downloadOutputNodeImages(nodeId){
    ...
    const mediaIds = (node?.images || []).map(item => item?.media_id).filter(Boolean);
    const useSaaSArchive = Boolean(window.SaaSCanvasGateway?.active);
    if(!node || (useSaaSArchive ? !mediaIds.length : !urls.length)){
        alert(tr('canvas.outputDownloadEmpty'));
        return;
    }
    try {
        const res = await fetch(useSaaSArchive ? '/api/v1/media/archive' : '/api/canvas-assets/download', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify(useSaaSArchive ? {media_ids:mediaIds} : {
                urls,
...
async function downloadGroupNodeImages(groupId){
    ...
    const mediaIds = items.map(item => item.media_id).filter(Boolean);
    const useSaaSArchive = Boolean(window.SaaSCanvasGateway?.active);
    if(useSaaSArchive && mediaIds.length !== items.length){
        alert(tr('canvas.outputDownloadEmpty'));
        return;
    }
    try {
...
async function uploadMediaFiles(files, point, onlyImages=false, opts={}){
    ...
    (data.files || []).forEach((file, i) => {
        ...
        const node = {
            ...
            name:file.name,
            mediaKind:kind,
            ...canvasUploadedMediaFields(file)
        };
        nodes.push(node);
        created.push(node);
    });
...
async function handleImageNodeDropEvent(e, nodeId, highlightEl){
    if(hasOutputImageDrag(e.dataTransfer)){
        ...
        setImageNodeFromOutput(nodeId, canvasOutputMediaFromDataTransfer(e.dataTransfer));
        return;
    }
...
async function fillImageNode(nodeId, files, opts={}){
    ...
    const node = nodes.find(n => n.id === nodeId);
    if(file && node){
        replaceCanvasImageNodeMedia(node, file, {mediaKind:file.kind || mediaKindForUpload(imgs[0])});
        render();
        scheduleSave();
    }
}
function setImageNodeFromOutput(nodeId, value){
    const node = nodes.find(n => n.id === nodeId);
    const item = canvasOutputMediaDragValue(value);
    const url = item.url;
    if(!node || node.type !== 'image' || !url || isVideoUrl(url) || isAudioUrl(url)) return;
    pushUndo();
    replaceCanvasImageNodeMedia(node, {...item, name:item.name || outputImageName(url), kind:'image'});
    render();
    scheduleSave();
}
...
function canvasUploadedMediaFields(file){
    const fields = {};
...
function replaceCanvasImageNodeMedia(node, file, extra={}){
    if(!node || !file) return node;
    node.url = file.url;
    node.name = file.name || node.name;
    node.mediaKind = file.kind || node.mediaKind || 'image';
    delete node.media_id;
    delete node.mime_type;
    delete node.mediaState;
    Object.assign(node, canvasUploadedMediaFields(file), extra);
    return node;
}
...
function addGeneratedImageNode(file, sourceNode, suffix, offsetY=0, extra={}){
    ...
    const next = {
        id:uid('img'), type:'image', x:p.x, y:p.y, url:file.url, name:file.name || suffix,
        ...canvasUploadedMediaFields(file), ...extra
    };
    nodes.push(next);
    selected.clear();
...
async function applyImageOutpaint(){
    ...
    if(file){
        replaceCanvasImageNodeMedia(node, file, {mediaKind:'image', natural_w:outW, natural_h:outH});
        closeImageEditor();
        render();
        scheduleSave();
    }
...
async function applyImageGridSplit(){
    ...
    if(files.length){
        ...
        const outputs = files.filter(file => file?.url);
        const layout = gridLayoutFromRects(rects);
        appendOutputImages(out, outputs, {url:node.url, name:node.name || 'source image'}, outputs.map((file, i) => ({
            runMs:0,
...
async function applyImageResize(){
    ...
    if(!file) return;
    replaceCanvasImageNodeMedia(node, file, {
        mediaKind:'image', natural_w:resized.targetW, natural_h:resized.targetH
    });
    closeImageEditor();
    render();
    scheduleSave();
...
function isNodeControl(target){
    return !!target.closest('textarea, input, select, option, button, audio, video, [contenteditable="true"], .seg, .gen-btn, .input-item, .blank-image, .mode-tabs, .ms-model-tabs, .llm-provider, .llm-output, .llm-chat-log, .llm-bubble, .llm-pane-resizer, .loop-preview');
}
...
function renderNode(node){
    ...
    el.className = `node ${node.type}-node ${node.unavailable ? 'unavailable-node' : ''} ${node.url ? 'has-image' : ''} ${hasFixedSize ? 'sized' : ''} ${selected.has(node.id) ? 'selected' : ''}`;
    ...
    if(node.unavailable){
        body.innerHTML = `<div class="unavailable-node-card"><i data-lucide="circle-slash-2"></i><strong>${langIsEn() ? 'Node unavailable' : '节点不可用'}</strong><span>${langIsEn() ? 'This node type is not supported. Its original data and connections are preserved.' : '当前版本不支持此节点；原始数据和连线已保留。'}</span></div>`;
    }
    el.appendChild(body);
    ...
    const canInput = ['generator','midjourney','output','llm','msgen','video','rh'].includes(node.type) || (node.type === 'loop' && (node.imageInput || node.showPrompt));
    const canOutput = ['image','prompt','loop','group','promptGroup','generator','midjourney','llm','msgen','video','rh','output'].includes(node.type);
    ...
    if(!node.unavailable) el.insertAdjacentHTML('beforeend', `<div class="resize-handle" title="${tr('canvas.resize')}"></div>`);
...
function bindOutputWrap(wrap, node){
    ...
    if(img){
        ...
        img.ondragstart = e => {
            ...
            const source = (node.images || []).find(item => outputUrlValue(item) === img.dataset.url);
            e.dataTransfer.setData('application/x-canvas-output-media', JSON.stringify(canvasOutputMediaDragValue(source || img.dataset.url)));
...
async function deleteCanvasAssetItem(itemId){
    const item = currentCanvasAssetItem(itemId);
    if(!item || !await centeredDeleteConfirm(`删除资产「${item.name || 'asset'}」？`)) return;
...
function renderCanvasAssetLibrary(){
    ...
    if(!activeCanvasAssetLibraryId || !libs.some(lib => lib.id === activeCanvasAssetLibraryId)) activeCanvasAssetLibraryId = canvasAssetLibrary.active_library_id || canvasAssetLibraries()[0]?.id || '';
    ...
    if(canvasAssetDropZone) {
        canvasAssetDropZone.style.display = 'flex';
...
async function deleteCanvasPromptTemplate(){
    const item = selectedCanvasPromptTemplate();
    if(!item) return;
    if(!await centeredDeleteConfirm(`删除提示词「${canvasPromptTemplateName(item) || '提示词'}」？`)) return;
    try {
...
async function deleteCanvasPromptTemplateGroup(groupId){
    const lib = activeCanvasPromptLibrary();
    if(lib && lib.id !== 'system'){
        if(!await centeredDeleteConfirm(tr('smart.tplDeleteGroupConfirm'))) return;
        try {
...
function renderLLMBody(node){
    ...
    if(window.SaaSCanvasGateway?.active && !chatApiProviders().length){
        wrap.innerHTML = `<div class="muted-note llm-saas-unavailable">尚未配置 LLM。<a href="/workspace/llm-settings">前往 LLM 设置</a></div>`;
        return wrap;
    }
    ...
    wrap.innerHTML = `
        ...
        <div class="gen-settings">
            ...
            <div class="gen-settings-row custom-size-row" style="display:none">
                <label class="field">
                    <div class="setting-title">${tr('canvas.width')}</div>
                    <input class="setting-input custom-w-input" type="number" min="256" max="8192" step="16" value="${escapeHtml(node.customWidth || '')}" placeholder="1024">
                </label>
                <label class="field">
                    <div class="setting-title">${tr('canvas.height')}</div>
                    <input class="setting-input custom-h-input" type="number" min="256" max="8192" step="16" value="${escapeHtml(node.customHeight || '')}" placeholder="1024">
                </label>
...
function renderVideoBody(node){
    ...
    if(window.SaaSCanvasGateway?.active){
        wrap.innerHTML = `<div class="muted-note video-saas-unavailable">视频节点数据已保留，等待平台管理员安全发布该能力。</div>`;
        return wrap;
    }
    const inputSources = generatorSources(node);
    const ordered = orderedSources(node, inputSources);
...
function randomFieldValue(field){
...
function rhFieldValue(node, field, media=null){
    ...
    if(rhRandomEnabled(field) && rhRandomActive(node, key)){
        ...
        if(node.rhRandomValues[key] === undefined){
            node.rhRandomValues[key] = randomFieldValue({
...
function renderRhBody(node){
    ...
    if(window.SaaSCanvasGateway?.active){
        wrap.innerHTML = `<div class="muted-note rh-saas-unavailable">节点数据已保留，等待平台管理员安全发布该能力。</div>`;
        return wrap;
    }
    node.rhParams = node.rhParams || {};
...
function rhModelSettingsHtml(node){
    ...
    return `
        <div class="gen-settings rh-model-settings">
            ...
            <div class="gen-settings-row custom-size-row" style="display:none">
                <label class="field"><div class="setting-title">${tr('canvas.width')}</div><input class="setting-input custom-w-input" data-rh-model-field="customWidth" type="number" min="256" max="8192" step="16" value="${escapeHtml(node.customWidth || '')}" placeholder="1024"></label>
                <label class="field"><div class="setting-title">${tr('canvas.height')}</div><input class="setting-input custom-h-input" data-rh-model-field="customHeight" type="number" min="256" max="8192" step="16" value="${escapeHtml(node.customHeight || '')}" placeholder="1024"></label>
                <div class="custom-size-guidance">宽高必须是 16 的倍数，范围 256–8192 像素</div>
            </div>
        </div>
    `;
...
function bindRhModelControls(wrap, node, media){
    ...
    const sync = () => {
        ...
        if(qualitySelect) qualitySelect.disabled = node.resolution === 'custom';
    ...
    if(rhRandomEnabled(field)){
        ...
        return `<div class="gen-settings-row rh-param-row ${wide ? 'wide' : ''}">
            <div class="random-field">
                ...
                <button class="tool-btn random-field-btn ${active ? 'active' : ''}" type="button" data-rh-random="${escapeAttr(key)}" title="${active ? '随机已开启，点击关闭' : '随机已关闭，点击开启'}"><i data-lucide="dice-5" class="w-4 h-4"></i></button>
            </div>
        </div>`;
    }
...
async function rhUploadValueIfNeeded(value, node=null){
    const text = String(value || '').trim();
    if(!text) return '';
    if(!/^https?:\/\//i.test(text) && !text.startsWith('/output/') && !text.startsWith('/assets/') && !text.startsWith('/api/media/')) return text;
...
async function runRhModelNode(node, opts={}){
    ...
    try {
        ...
        if(await completeSaaSGenerationSubmission(node, out, taskInfos)) return;
        if(!out){
            let outputs = [];
...
const CANVAS_GENERATOR_TYPES = ['generator','midjourney','msgen','video','rh'];
const CANVAS_IMAGE_OUTPUT_TYPES = ['generator','midjourney','msgen','rh'];
    ...
    return (node?.generatedOutputs || [])
        .map((item, i) => {
            ...
            return {
                ...
                kind,
                index:i,
                ...canvasUploadedMediaFields(item)
            };
        })
        .filter(Boolean)
...
function mediaRefsFromNode(node){
    ...
    if(node.type === 'image' && node.url){
        ...
        return [{
            ...
            role:node.role || '',
            kind,
            ...canvasUploadedMediaFields(node)
        }];
    }
    if(node.type === 'group'){
        return (node.items || [])
            ...
            .map(item => ({
                url:item.url,
                name:item.name || mediaKindForNode(item),
                role:item.role || '',
    ...
    return connections.filter(c => c.to === gen.id).map(c => nodes.find(n => n.id === c.from)).filter(Boolean).map(n => {
        if(n.type === 'output' && (n.images||[]).length){
            ...
            if(found){
                ...
                return {id:n.id, type:'outputImage', label:'上游输出', preview:last, refs:[{
                    url:last,
                    name:'output.png',
                    role:found.item?.role || '',
                    kind,
                    nodeId:n.id,
                    outputIndex:found.index,
                    ...canvasUploadedMediaFields(found.item)
                }], prompt:''};
            }
        }
        ...
        if(n.type === 'image' && n.url) {
            ...
            if(directRef.role === 'mask'){
                const originalRefs = connections
                    .filter(connection => connection.to === n.id)
                    .map(connection => nodes.find(candidate => candidate.id === connection.from))
                    .flatMap(mediaRefsFromNode)
                    .filter(ref => ref.role !== 'mask' && mediaKindForRef(ref) === 'image');
                return {
                    id:n.id,
                    type:kind,
                    label:n.name || kind,
                    preview:n.url,
                    refs:[...originalRefs, directRef],
                    prompt:''
                };
            }
            return {id:n.id, type:kind, label:n.name || kind, preview:n.url, refs:[directRef], prompt:''};
        }
        if(n.type === 'group') {
            ...
            const sources = items.filter(x => x.type === 'image' && x.url).map(img => ({
                ...
                refs:[{
                    url:img.url,
                    name:img.name || mediaKindForNode(img),
                    role:img.role || '',
                    kind:mediaKindForNode(img),
                    ...canvasUploadedMediaFields(img)
                }],
                prompt:''
            }));
...
async function runGenerator(genId, opts={}){
    ...
    try {
        ...
        if(await completeSaaSGenerationSubmission(gen, out, taskInfos)) return;
        if(!out){
            let outputs = [];
            for(const task of taskInfos){
                const result = await waitCanvasImageTaskResult(task.task_id, {cascadeTargetId});
...
function canvasRunTypes(){
    return ['generator','midjourney','msgen','llm','video','rh'];
}
...
function isExpiredAssetUrl(url){
    return Boolean(url && expiredAssetUrls.has(url));
}
function missingAssetHtml(url, compact=false){
    const expired = isExpiredAssetUrl(url);
    const presentation = StudioMedia.unavailableMediaPresentation({expired, language:langIsEn() ? 'en' : 'zh'});
    return `<div class="missing-asset ${expired ? 'expired' : ''} ${compact ? 'compact' : ''}" title="${escapeAttr(presentation.message)}"><i data-lucide="${presentation.icon}" class="${compact ? 'w-4 h-4' : 'w-6 h-6'}"></i><span>${escapeHtml(presentation.message)}</span></div>`;
}
...
const GENERATION_LOG_RETENTION_MS = 24 * 60 * 60 * 1000;
function pruneGenerationLogs(){
    if(!canvas) return false;
    const logs = Array.isArray(canvas.logs) ? canvas.logs : [];
    const cutoff = Date.now() - GENERATION_LOG_RETENTION_MS;
    const retained = logs.filter(log => Number(log?.createdAt || 0) >= cutoff);
    const changed = retained.length !== logs.length;
    canvas.logs = retained;
    return changed;
}
...
function renderCanvasLog(){
    if(pruneGenerationLogs()) scheduleSave();
...
function mergeGeneratedOutputs(node, outputs, append=false){
    ...
    const clean = (outputs || []).map(item => {
        ...
        if(kind === 'image') return item && typeof item === 'object' ? {...item, url, kind} : url;
        return {url, kind};
    }).filter(Boolean);
    if(!append){
...
async function prepareSaaSGenerationSubmission(node){
    if(!window.SaaSCanvasGateway?.active || !node) return '';
    const taskId = window.SaaSCanvasGateway.newGenerationTaskId?.();
    if(!taskId) return '';
    const taskIds = [...new Set([
        ...(Array.isArray(node.saasGenerationTaskIds) ? node.saasGenerationTaskIds : []),
        taskId
    ].filter(Boolean))];
    node.saasGenerationTaskIds = taskIds;
    node.lastGenerationTaskId = taskId;
    scheduleSave();
    await flushCanvasSave();
    return taskId;
}
async function rollbackPreparedSaaSGenerationSubmission(node, taskId){
    if(!node || !taskId) return;
    const taskIds = (Array.isArray(node.saasGenerationTaskIds) ? node.saasGenerationTaskIds : [])
        .filter(id => id !== taskId);
    if(taskIds.length) node.saasGenerationTaskIds = taskIds;
    else delete node.saasGenerationTaskIds;
    if(node.lastGenerationTaskId === taskId){
        if(taskIds.length) node.lastGenerationTaskId = taskIds[taskIds.length - 1];
        else delete node.lastGenerationTaskId;
    }
    scheduleSave();
    await flushCanvasSave();
}
async function createCanvasImageTask(payload, options={}){
    const res = await cascadeFetch('/api/canvas-image-tasks', {
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({...payload, canvas_id:canvas?.id || ''})
    }, options);
    if(!res.ok) throw new Error(await responseErrorMessage(res, tr('canvas.generationFailed')));
    return res.json();
}
async function createCanvasImageTasks(payload, count, options={}){
    if(window.SaaSCanvasGateway?.active){
        const taskId = await prepareSaaSGenerationSubmission(options.bindingNode);
        try {
            return [await createCanvasImageTask({...payload, quantity:count, task_id:taskId}, options)];
        } catch(error) {
            await rollbackPreparedSaaSGenerationSubmission(options.bindingNode, taskId);
            throw error;
        }
    }
    return Promise.all(Array.from({length:count}, () => createCanvasImageTask(payload, options)));
}
async function completeSaaSGenerationSubmission(node, out, taskInfos){
    const task = (taskInfos || []).find(item => item?.saas_generation_task);
    if(!window.SaaSCanvasGateway?.active || !task) return false;
    const taskIds = (taskInfos || []).map(item => item?.task_id).filter(Boolean);
    node.saasGenerationTaskIds = [...new Set([
        ...(Array.isArray(node.saasGenerationTaskIds) ? node.saasGenerationTaskIds : []),
        ...taskIds
    ])];
    ...
    if(out){
        ...
        const pending = taskIds.filter(taskId => !currentIds.has(taskId)).map(taskId => ({
            id:`saas-pending-${taskId}`,
            canvasTaskId:taskId,
            canvasTaskType:'online-image',
            startedAt:Date.now(),
            run:{node:{id:node.id}, request:{task_id:taskId}, refs:[]}
        }));
    ...
    scheduleSave();
    await saveCanvas();
    taskIds.forEach(taskId => { void pollCanvasImageTask(taskId); });
    return true;
}
...
async function pollCanvasImageTask(taskId, options={}){
    ...
    try {
        if(window.SaaSCanvasGateway?.streamGenerationTask){
            ...
            failCanvasImageTask(taskId, task.failure_message || tr('canvas.generationFailed'), task);
            return 'failed';
        }
        while(true){
...
function appendOutputImages(out, images, compareRef, metas=[], layout=null){
    ...
    out.images = [...(out.images || []), ...list.map((url, i) => {
        ...
        Object.assign(item, canvasUploadedMediaFields(source));
    ...
    if(compareRef?.url){
        ...
        list.forEach(item => {
            out.imageComparisons[outputUrlValue(item)] = {url:compareRef.url, name:compareRef.name || 'input image'};
        });
    }
}
...
function canvasOutputMediaDragValue(value){
    const source = value && typeof value === 'object' ? value : {url:value};
    const item = {
        url:source.url || '',
        name:source.name || '',
        kind:source.kind || source.mediaKind || 'image',
        ...canvasUploadedMediaFields(source)
    };
    return Object.fromEntries(Object.entries(item).filter(([, field]) => field !== '' && field != null));
}
function canvasOutputMediaFromDataTransfer(dataTransfer){
    const encoded = dataTransfer?.getData?.('application/x-canvas-output-media') || '';
    if(encoded){
        try { return canvasOutputMediaDragValue(JSON.parse(encoded)); } catch(_error){}
    }
    return canvasOutputMediaDragValue(dataTransfer?.getData?.('application/x-canvas-output-image') || '');
}
function createImageCardFromOutput(value, point){
    const item = canvasOutputMediaDragValue(value);
    const url = item.url;
    if(!ensureCanvas() || !url) return;
    ...
    const p = point || defaultPoint(0, 0);
    nodes.push({
        id:uid('img'), type:'image', x:p.x, y:p.y, url,
        name:item.name || outputImageName(url), mediaKind:'image', ...canvasUploadedMediaFields(item)
    });
    render();
    scheduleSave();
}
...
canvasAssetDropZone?.addEventListener('drop', async event => {
    ...
    try {
        if(hasOutputImageDrag(event.dataTransfer)){
            const item = canvasOutputMediaFromDataTransfer(event.dataTransfer);
...
assetManagerModal?.addEventListener('click', async event => {
    ...
    if(workflowRemove){
        ...
        const item = (activeCanvasWorkflowCategory()?.items || []).find(entry => entry.id === itemId);
        if(!item || !await centeredDeleteConfirm(`删除工作流「${item.name || 'workflow'}」？`)) return;
        const data = await fetch(`/api/asset-library/items/${encodeURIComponent(item.id)}`, {method:'DELETE'}).then(r => r.json());
        canvasAssetLibrary = data.library || canvasAssetLibrary;
        managerSelectedWorkflowIds.delete(item.id);
        renderAssetManager(); renderCanvasAssetLibrary(); return;
    ...
    if(promptRemove){
        ...
        const item = (lib.items || []).find(entry => entry.id === itemId);
        if(!item || !await centeredDeleteConfirm(`删除提示词「${item.name || '提示词'}」？`)) return;
    ...
    if(event.target.closest?.('[data-manager-asset-lib-delete]')){
        const lib = activeCanvasAssetLibrary();
        if(!lib || !await centeredDeleteConfirm(`删除资产库「${lib.name || '资产库'}」？`)) return;
    ...
    if(event.target.closest?.('[data-manager-asset-cat-delete]')){
        const cat = activeCanvasMediaCategory();
        if(!cat || !await centeredDeleteConfirm(`删除分组「${cat.name || '分组'}」？`)) return;
    ...
    if(event.target.closest?.('[data-manager-prompt-lib-delete]')){
        const lib = activeCanvasPromptLibrary();
        if(!lib || lib.readonly || !await centeredDeleteConfirm(`删除提示词库「${lib.name || '提示词库'}」？`)) return;
...
async function downloadUrl(url, filename='download'){
    ...
    const link = document.createElement('a');
    link.href = href;
    link.download = filename || outputDownloadName(raw);
    document.body.appendChild(link);
    link.click();
    link.remove();
    setTimeout(() => URL.revokeObjectURL(href), 1200);
...
board.addEventListener('drop', async e => {
    ...
    if(hasOutputImageDrag(e.dataTransfer)) {
        createImageCardFromOutput(canvasOutputMediaFromDataTransfer(e.dataTransfer), screenToWorld(e.clientX, e.clientY));
        return;
    }
...
function hasOutputImageDrag(dataTransfer){
    ...
    return types.includes('application/x-canvas-output-media') || types.includes('application/x-canvas-output-image');
}
