...
smartLogToggle?.addEventListener('click', event => {
...
let nodes = [];
const expiredSmartMediaUrls = new Set();
let selectedId = '';
let selectedIds = [];
...
function smartOriginalMediaUrl(itemOrUrl){
    return StudioMedia.originalMediaUrl(itemOrUrl);
}
...
function smartSettingsModeKey(source=settings){
    const engine = ['api','volcengine','modelscope','runninghub'].includes(source?.engine) ? source.engine : 'api';
...
function applyRecentSmartSettingsForCurrentMode(){
    const requestedEngine = ['api','volcengine','modelscope','runninghub'].includes(settings.engine) ? settings.engine : 'api';
...
function normalizeLegacySmartNode(node){
    ...
    if(node.unavailable) return node;
...
function withOutpaintDisplaySettings(node, baseSettings){
    ...
    const engine = ['api','volcengine','modelscope','runninghub'].includes(baseSettings?.engine) ? baseSettings.engine : 'api';
    const next = {
        ...baseSettings,
        resolution:'custom',
        ratio:'',
...
function validSmartCustomPixelSize(width, height){
    return [Number(width), Number(height)].every(value => Number.isInteger(value) && value >= 256 && value <= 8192 && value % 16 === 0);
}
function validateSmartCustomPixelSize(runSettings=settings){
    const customSize = runSettings?.engine === 'modelscope'
        ? runSettings.msResolution === 'custom'
        : runSettings?.resolution === 'custom';
    if(!customSize) return true;
    const width = runSettings?.engine === 'modelscope' ? runSettings.msCustomWidth : runSettings.customWidth;
    const height = runSettings?.engine === 'modelscope' ? runSettings.msCustomHeight : runSettings.customHeight;
    if(validSmartCustomPixelSize(width, height)) return true;
    showSmartSizeNotice();
    return false;
}
function showSmartSizeNotice(){
    let dialog = document.getElementById('smartSizeNotice');
    if(!dialog){
        dialog = document.createElement('dialog');
        dialog.id = 'smartSizeNotice';
        dialog.className = 'smart-size-notice';
        dialog.innerHTML = '<form method="dialog"><div class="smart-size-notice-mark">!</div><div><h3>尺寸提示</h3><p>自定义像素尺寸不对，请修改！</p></div><button type="submit">我知道了</button></form>';
        document.body.appendChild(dialog);
    }
    dialog.showModal();
}
...
function renderDynamicParams(){
    ...
    settings.engine = ['api','volcengine','modelscope','runninghub'].includes(settings.engine) ? settings.engine : 'api';
...
function renderApiVideoParams(){
    if(window.SaaSCanvasGateway?.active){
        dynamicParams.innerHTML = `<div class="muted-note video-saas-unavailable">视频配置已保留，等待平台管理员安全发布该能力。</div>`;
        return;
    }
...
function renderRunningHubParams(){
    if(window.SaaSCanvasGateway?.active){
        dynamicParams.innerHTML = `<div class="muted-note rh-saas-unavailable">节点数据已保留，等待平台管理员安全发布该能力。</div>`;
        return;
    }
...
function renderModelControl(models){
    ...
    return `<div class="smart-control model-control">
        <button class="smart-pill" type="button"><i data-lucide="sparkles"></i><span class="sub">${escapeHtml(settings.model ? modelLabel(settings.model) : tr('smart.model'))}</span></button>
        <div class="smart-popover compact-popover">
            <div class="smart-popover-title">${escapeHtml(tr('smart.imageModel'))}</div>
            <div class="model-list">
                ${models.map(m => `<button type="button" class="direct-option ${m === settings.model ? 'active' : ''}" data-smart-param="model" data-smart-value="${escapeHtml(m)}"><span>${escapeHtml(modelLabel(m))}</span></button>`).join('') || `<div class="muted-note">${escapeHtml(tr('smart.noImageModel'))}</div>`}
            </div>
        </div>
    </div>`;
}
        ...
        <div class="smart-popover size-picker-popover">
            ...
            ${scope === 'custom' ? `<div class="size-picker-pane size-picker-custom">
                <div class="size-custom-box">
                    ...
                    <div class="size-custom-guidance">宽高必须是 16 的倍数，范围 256–8192 像素</div>
                    <div class="size-custom-row"><input type="number" min="256" max="8192" step="16" data-param="${wKey}" value="${escapeHtml(settings[wKey] || '')}" placeholder="宽度"><span>×</span><input type="number" min="256" max="8192" step="16" data-param="${hKey}" value="${escapeHtml(settings[hKey] || '')}" placeholder="高度"></div>
                </div>
            </div>` : ''}
        </div>
    </div>`;
...
function renderQualityControl(){
    ...
    const disabled = settings.resolution === 'custom';
    const labels = {auto:tr('smart.qualityAuto'), low:tr('smart.qualityLow'), medium:tr('smart.qualityMid'), high:tr('smart.qualityHigh')};
    return `<div class="smart-control quality-control ${disabled ? 'is-disabled' : ''}">
        <button class="smart-pill" type="button" ${disabled ? 'disabled title="自定义尺寸下清晰度不可选"' : ''}><i data-lucide="sliders-horizontal"></i><span>${escapeHtml(labels[value] || value)}</span></button>
        <div class="smart-popover compact-popover">
...
function randomSmartFieldValue(field){
...
function smartCanvasManagedMediaUrls(){
    ...
    const visit = value => {
        if(typeof value === 'string'){
            if(value.startsWith('/api/media/')) urls.add(value);
            return;
        }
    ...
    nodes.forEach(node => {
        visit(node?.images || []);
        visit(node?.inputImage || null);
    });
    visit(canvas?.logs || []);
    return [...urls].slice(0, 3000);
}
async function refreshSmartMediaAvailability(){
    const urls = smartCanvasManagedMediaUrls();
    if(!urls.length){
        const changed = expiredSmartMediaUrls.size > 0;
        expiredSmartMediaUrls.clear();
        return changed;
    }
    try {
        const availability = await StudioMedia.checkMediaAvailability(urls);
        const nextExpired = availability.expired;
        const changed = nextExpired.size !== expiredSmartMediaUrls.size
            || [...nextExpired].some(url => !expiredSmartMediaUrls.has(url));
        expiredSmartMediaUrls.clear();
        nextExpired.forEach(url => expiredSmartMediaUrls.add(url));
        return changed;
    } catch(e) {
        // 检查失败不把仍可显示的媒体误标为过期；下次打开或远端同步时重试。
        return false;
    }
}
function smartMediaIsExpired(itemOrUrl){
    return expiredSmartMediaUrls.has(smartOriginalMediaUrl(itemOrUrl));
}
...
function smartNodeHasDisplayResult(node){
    return Boolean(Array.isArray(node?.images) && node.images.some(img => img?.url && !img.loopInputPreview));
}
...
function clearCompletedSourceBusyStates(){
    let changed = false;
    (nodes || []).forEach(node => {
        if(!node?.unavailable && clearSourceBusyStateIfDownstreamDone(node)) changed = true;
    });
    return changed;
}
function hideCompletedRunTimers(){
    let changed = false;
    (nodes || []).forEach(node => {
        if(!node || node.unavailable || node.type === 'smart-prompt') return;
...
function clearCompletedNodeBusyStates(){
    let changed = false;
    (nodes || []).forEach(node => {
        if(!node || node.unavailable || !smartNodeHasCompletedResult(node) || !smartNodeInFlight(node)) return;
...
function applyMergedServerCanvas(serverCanvas){
    ...
    render();
    refreshSmartMediaAvailability().then(changed => { if(changed) render(); });
...
function renderAssetLibrary(){
    ...
    if(!activeAssetLibraryId || !libs.some(lib => lib.id === activeAssetLibraryId)) activeAssetLibraryId = assetLibrary.active_library_id || assetLibraries()[0]?.id || '';
    ...
    const smartClassCats = imageMode ? assetSmartClassEntries().map(entry => ({
        ...entry,
        id:entry.id,
    ...
    if(assetRenameCategoryBtn) assetRenameCategoryBtn.disabled = !cat || Boolean(smartClass);
    ...
    `).join('') : `<div class="asset-empty">${escapeHtml(smartClass ? '这个智能分类下暂无素材' : (workflowMode ? '暂无工作流资产' : tr('smart.assetEmpty')))}</div>`;
...
async function loadCanvas(){
    ...
    try {
        ...
        const data = await res.json();
        canvas = data.canvas;
        const prunedExpiredLogs = pruneSmartGenerationLogs();
...
function smartExpiredMediaHtml(width='', height='', compact=false){
    const style = width && height ? ` style="width:${Number(width)}px;height:${Number(height)}px"` : '';
    const presentation = StudioMedia.unavailableMediaPresentation({expired:true, language:window.StudioI18n?.lang() || 'zh'});
    return `<div class="media-expired-card ${compact ? 'compact' : ''}"${style} title="${escapeAttr(presentation.message)}"><i data-lucide="${presentation.icon}"></i><span>${escapeHtml(presentation.message)}</span></div>`;
}
function thumbMediaHtml(img){
    if(smartMediaIsExpired(img)) return smartExpiredMediaHtml('', '', true);
...
function singleMediaHtml(img, w, h){
    if(smartMediaIsExpired(img)) return smartExpiredMediaHtml(w, h);
...
function proxiedMediaUrl(itemOrUrl, name=''){
    return StudioMedia.proxiedMediaUrl(itemOrUrl, name, {proxyUnknown:true});
}
function displayMediaUrl(itemOrUrl, name=''){
    return StudioMedia.displayMediaUrl(itemOrUrl, name);
}
...
async function downloadPreviewImage(){
    ...
    if(!image?.url) return;
    try {
        await downloadMediaItem(image, 'image');
    } catch(e) {
        toast((e.message || '下载失败').slice(0, 160));
    }
}
async function downloadPreviewFile(item){
    if(!item?.url) return;
    try {
        await downloadMediaItem(item, 'output');
    } catch(e) {
        toast((e.message || '下载失败').slice(0, 160));
    }
}
...
async function zipDownloadImageItems(title, items){
    ...
    try {
        ...
        const mediaIds = list.map(item => item.media_id).filter(Boolean);
        const useSaaSArchive = Boolean(window.SaaSCanvasGateway?.active);
        if(useSaaSArchive && mediaIds.length !== list.length) throw new Error('部分图片尚未保存到账户媒体');
        const response = await fetch(useSaaSArchive ? '/api/v1/media/archive' : '/api/canvas-assets/download', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
...
function smartRunRequestMeta(run){
    ...
    const taskMeta = taskIds.length ? {task_id:taskIds.join(', ')} : {};
    if(s.engine === 'modelscope') return {...taskMeta, backend:'Modelscope', model:s.msgenModel || '', custom_model:s.msCustomModel || ''};
    if(run?.kind === 'video') return {...taskMeta, provider_id:s.videoProvider || '', model:s.videoModel || '', duration:s.videoDuration || '', aspect_ratio:s.videoAspect || '', resolution:s.videoResolution || ''};
    return {...taskMeta, provider_id:s.provider_id || '', model:s.model || '', size:run?.size || '', quality:s.quality || '', n:s.count || 1};
}
...
const SMART_GENERATION_LOG_RETENTION_MS = 24 * 60 * 60 * 1000;
function pruneSmartGenerationLogs(){
    if(!canvas) return false;
    const logs = Array.isArray(canvas.logs) ? canvas.logs : [];
    const cutoff = Date.now() - SMART_GENERATION_LOG_RETENTION_MS;
...
function rememberSmartPendingLog(node, run, taskIds=[]){
    if(!node || !run) return;
    node.pendingLogRun = {...run, taskIds:[...(taskIds || [])].filter(Boolean)};
    node.pendingLogStartedAt = nowMs();
}
function restoredSmartPendingLogContext(node){
    if(!node) return {};
    const pendingTaskIds = smartPendingTasks(node).map(task => task.taskId).filter(Boolean);
    const storedRun = node.pendingLogRun;
    const run = storedRun ? {...storedRun, taskIds:[...new Set([...(storedRun.taskIds || []), ...pendingTaskIds])]} : {
        nodeId:node.runMeta?.sourceNodeId || node.id || '',
        nodeType:node.type || 'smart-image',
        kind:node.outputKind || 'image',
        settings:cloneSmartSettings(node.runSettings || settings),
        prompt:node.runPrompt || node.runModelPrompt || node.runMeta?.prompt || '',
        refs:(node.runInputRefs || node.runPromptRefs || []).map(ref => ({url:ref.url || '', name:ref.name || 'image', kind:ref.kind || ''})).filter(ref => ref.url),
        taskIds:pendingTaskIds
    };
    return {run, runLogStart:Number(node.pendingLogStartedAt || node.runStartedAt || nowMs())};
}
function clearSmartPendingLog(node){
    if(!node) return;
    delete node.pendingLogRun;
    delete node.pendingLogStartedAt;
}
...
function renderSmartCanvasLog(){
    if(pruneSmartGenerationLogs()) scheduleSave();
...
function openSmartCanvasLog(){
    const modal = document.getElementById('smartLogModal') || smartLogModal;
    const list = document.getElementById('smartLogList') || smartLogList;
    modal?.classList.add('open');
    if(list && !list.innerHTML) list.innerHTML = `<div class="log-empty">${escapeHtml(tr('canvas.noLogs'))}</div>`;
    try {
        renderSmartCanvasLog();
    } catch(err) {
        console.error('renderSmartCanvasLog failed', err);
        if(list) list.innerHTML = `<div class="log-empty">${escapeHtml(err?.message || String(err))}</div>`;
    }
}
function closeSmartCanvasLog(){
    (document.getElementById('smartLogModal') || smartLogModal)?.classList.remove('open');
}
...
function promptNodeBodyHtml(node){
    const saasLlmUnavailable = Boolean(window.SaaSCanvasGateway?.active && !chatApiProviders().length);
    if(!saasLlmUnavailable){
    ...
    const llmParams = node.llmEnabled ? (saasLlmUnavailable
        ? `<div class="muted-note llm-saas-unavailable">尚未配置 LLM。<a href="/workspace/llm-settings">前往 LLM 设置</a></div>`
        : `
...
function nodeBodyHtml(node, layout){
    if(node.unavailable){
        ...
        return `<div class="unavailable-node-card"><i data-lucide="circle-slash-2"></i><strong>${isEn ? 'Node unavailable' : '节点不可用'}</strong><span>${isEn ? 'This node type is not supported. Its original data and connections are preserved.' : '当前版本不支持此节点；原始数据和连线已保留。'}</span></div>`;
    }
    ...
    if(node.pending && imgs.length === 0){
        const count = Math.max(1, Number(node.pending) || 1);
        if(count <= 1) return `<div class="loading-cell single generation-state-cell" style="width:${layout.width}px;height:${layout.height}px"><strong>正在生图</strong><span>请求已发送至上游</span></div>`;
        const cols = Math.min(4, Math.max(2, Math.ceil(Math.sqrt(count))));
        const rows = Math.ceil(count / cols);
...
function smartNodeToolbarHtml(node){
    ...
    const actions = [
        ...
        {key:'keep', icon:'bookmark-plus', label:'保留到画布', enabled:isRetainableSmartCanvasMedia(item)},
...
async function promoteSmartCanvasMedia(item){
    if(window.SaaSCanvasGateway?.active && item?.media_id){
        ...
        return {...await response.json(), url:item.url};
    }
    return StudioMedia.promoteMedia(item, {ownerId:canvasId});
}
async function keepSmartNodeMedia(node, imageIndex){
    const current = node?.images?.[imageIndex];
    const item = imageForDisplay(current);
    if(!item?.url) return;
    try {
        const media = await promoteSmartCanvasMedia(item);
        node.images[imageIndex] = typeof current === 'string'
            ? {url:media.url || item.url, mediaState:media.state || 'persistent'}
            : {...current, url:media.url || item.url, originalLocalUrl:media.url || item.url, mediaState:media.state || 'persistent'};
        render();
        scheduleSave();
        toast('已保留到画布');
    } catch(error) {
        toast(error.message || '保留媒体失败');
    }
}
...
function render(){
    ...
    const nodeHtmlEntries = nodes
        ...
        const title = node.unavailable ? `${window.StudioI18n?.lang?.() === 'en' ? 'Unsupported' : '暂不支持'} · ${escapeHtml(unavailableType)}` : node.type === 'smart-group' ? (node.title === '万能分组' ? '智能分组' : (node.title || '智能分组')) : node.type === 'smart-prompt' ? 'Prompt' : node.type === 'smart-loop' ? 'Loop' : (imgs.length > 1 ? 'Group' : imgs.length ? 'Image' : escapeHtml(tr('smart.createImportNode')));
        ...
        const isImageNode = node.type === 'smart-image' || !node.type;
        const isQueued = Boolean(node.queued && imgs.length === 0);
        ...
        const html = `<div class="image-node ${node.unavailable ? 'unavailable-node' : ''} ${isEmpty ? 'empty-node' : ''} ${isGroup ? 'group-node' : ''} ${isHistory ? 'history-group-node' : ''} ${isPrompt ? 'prompt-smart-node' : ''} ${isLoop ? 'loop-smart-node' : ''} ${isSmartGroup ? 'smart-group-node' : ''} ${isCompactMember ? 'smart-group-member-node' : ''} ${isNodeSelected(node.id) ? 'selected' : ''} ${(dragState?.groupIds?.includes(node.id) || dragState?.id === node.id) ? 'dragging' : ''} ${node.running ? 'node-running' : ''} ${isPending ? 'node-pending' : ''}" data-id="${escapeHtml(node.id)}" style="left:${node.x || 0}px;top:${node.y || 0}px;width:${layout.width}px;height:${layout.height}px">
            ...
            ${node.unavailable ? '' : `${smartNodeToolbarHtml(node)}${smartGroupToolbarHtml(node)}`}
            ${node.unavailable ? '' : runTimePillHtml(node)}
            <div class="node-body">${body}</div>
            ${isCompactMember && (isPrompt || isLoop) ? '<div class="smart-group-member-grab" title="拖动移出分组"></div>' : ''}
            <div class="node-hint">${hint}</div>
            ${!node.unavailable && (imgs.length || node.pending || isQueued || isPrompt || isLoop || isSmartGroup) ? '<div class="node-resize-handle" data-resize="1"></div>' : ''}
            ${node.unavailable ? '' : '<div class="node-port port-in" data-port="in" title="input"></div><div class="node-port port-out" data-port="out" title="output"></div>'}
        </div>`;
        return {node, html};
    });
    const tpl = document.createElement('template');
...
function smartCanvasUploadedMediaFields(file){
    const fields = {};
...
function replaceEditedImage(file, extra={}){
    ...
    const next = {
        ...(node.images[index] || {}), url:file.url, name:file.name,
        kind:file.kind || mediaKindForItem(file), natural_w:0, natural_h:0
    };
    delete next.media_id;
    delete next.mime_type;
    delete next.mediaState;
    node.images[index] = {...next, ...smartCanvasUploadedMediaFields(file), ...extra};
...
async function applyImageMask(){
    ...
    if(file){
        node.images.push({url:file.url, name:file.name, role:'mask', ...smartCanvasUploadedMediaFields(file)});
...
function maskCanvasFromDrawCanvas(src){
    ...
    for(let i = 0; i < srcData.data.length; i += 4){
        ...
        out.data[i + 1] = 0;
        out.data[i + 2] = 0;
        // OpenAI Images edits transparent pixels and preserves opaque pixels.
...
function buildPromptRequest(node, overrideDefaultImages=null, consumeDefault=false, ctx=smartLoopContext){
    ...
    let ordinaryImageIndex = 0;
    const refs = defaultRefs.map(img => ({
        ...img,
        role:img.role === 'mask' ? 'mask' : `image_${++ordinaryImageIndex}`
    }));
    ...
    parts.forEach(part => {
        ...
        if(!refMap.has(part.url)){
            ...
            refs.push({
                ...
                role:`image_${++ordinaryImageIndex}`
            });
        }
...
async function generateUrlsForCurrentSettings(node, prompt, refs, runSettings=settings){
    ...
    if(activeSettings.engine === 'runninghub' && runningHubSelectedModel(activeSettings)){
        ...
        if(rememberSaaSGenerationSubmission(node, taskResult)) return {urls:[], kind:'image', saas_generation_task:true, taskIds:taskResult.taskIds};
...
async function runCascadeStepIntoNode(sourceNode, targetNode, inputRefs, ctx=smartLoopContext){
    ...
    try {
        ...
        clearSmartPendingLog(outputNode);
        ...
        if(!smartPendingTasks(outputNode).length) clearSmartPendingLog(outputNode);
        render();
        throw e;
    }
}
async function runLoopRoundIntoSlot(loopNode, rootNode, outputSlot, loopIndex, ctx){
    ...
    try {
        ...
        if(isApiLikeEngine(runSettings.engine) && runSettings.apiKind !== 'video'){
            rememberSmartPendingLog(outputSlot, runLog);
            const taskResult = await runApiGeneration(prompt, request.refs || [], runSettings, outputSlot);
            if(rememberSaaSGenerationSubmission(outputSlot, taskResult)){
                outputSlot.queued = false;
                await saveCanvas();
                return [];
            }
            ...
            if(smartRecoverableImageTask(outputSlot)){
                outputSlot.queued = false;
                return [];
            }
...
async function runGeneration(){
    ...
    if(!validateSmartCustomPixelSize(settings)){
        settings = previousSettings;
        return;
    }
    ...
    render();
    try {
        rememberSmartPendingLog(pendingNode, runLog);
        if(isApiLikeEngine(settings.engine) && settings.apiKind === 'video'){
            ...
            clearSmartPendingLog(pendingNode);
        ...
        if(isApiLikeEngine(settings.engine) || rhModelMode){
            if(rememberSaaSGenerationSubmission(pendingNode, outImages)){
        ...
        delete pendingNode._runMetaTargetId;
        if(!e?.smartGenerationLogged) addSmartGenerationLog({run:runLog, outputs:[], runMs:nowMs() - runLogStart, error:e.message || String(e)});
        if(!smartPendingTasks(pendingNode).length) clearSmartPendingLog(pendingNode);
...
async function runApiGeneration(prompt, refs, runSettings=settings, bindingNode=null){
    ...
    const submit = body => fetch('/api/canvas-image-tasks', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body)}).then(async r => {
        if(!r.ok) throw new Error(await r.text());
        return r.json();
    });
    if(window.SaaSCanvasGateway?.active){
        const taskId = await prepareSaaSGenerationSubmission(bindingNode);
        let task;
        try {
            task = await submit({...payload, quantity:count, task_id:taskId});
        } catch(error) {
            await rollbackPreparedSaaSGenerationSubmission(bindingNode, taskId);
            throw error;
        }
        const taskIds = task.task_id ? [task.task_id] : [];
        return {taskIds, count, saas_generation_task:true, status_url:task.status_url};
    }
    const tasks = await Promise.all(Array.from({length:count}, () => submit(payload)));
    return {taskIds:tasks.map(task => task.task_id).filter(Boolean), count, providerId:payload.provider_id, model:payload.model};
}
function rememberSaaSGenerationSubmission(node, taskResult){
    if(!window.SaaSCanvasGateway?.active || !taskResult?.saas_generation_task) return false;
    const taskIds = Array.isArray(taskResult.taskIds) ? taskResult.taskIds.filter(Boolean) : [];
    node.saasGenerationTaskIds = [...new Set([
        ...(Array.isArray(node.saasGenerationTaskIds) ? node.saasGenerationTaskIds : []),
        ...taskIds
    ])];
    node.lastGenerationTaskId = taskIds[0] || '';
    const currentTasks = smartPendingTasks(node);
    const currentTaskIds = new Set(currentTasks.map(task => task.taskId));
    const quantity = Math.max(1, Math.trunc(Number(taskResult.count) || 1));
    node.pendingTasks = [
        ...currentTasks,
        ...taskIds.filter(taskId => !currentTaskIds.has(taskId)).map(taskId => ({taskId, kind:'image', quantity}))
    ];
    if(node.pendingLogRun) node.pendingLogRun = {...node.pendingLogRun, taskIds:[...new Set([...(node.pendingLogRun.taskIds || []), ...taskIds])]};
    node.pending = smartPendingTasks(node).reduce((total, task) => total + smartPendingTaskQuantity(task), 0);
    node.running = false;
    node.queued = true;
    render();
    scheduleSave();
    void resumeSmartPendingNode(node);
    return true;
}
...
function smartPendingTaskQuantity(task){
    return Math.max(1, Math.trunc(Number(task?.quantity) || 1));
}
...
function finalizeSmartPendingTask(node, taskId, images, kind='image'){
    ...
    node.pending = Math.max(0, Number(node.pending || 0) - smartPendingTaskQuantity(completedTask));
    const ext = kind === 'video' ? 'mp4' : kind === 'audio' ? 'mp3' : kind === 'text' ? 'txt' : 'png';
    ...
    const additions = cleanHistoryImages((mediaItems || []).map((item, i) => {
        ...
        return stripImageGenerationMeta(copyMediaSizeFields(item, {
            ...
            kind:itemKind,
            ...smartCanvasUploadedMediaFields(item),
            generatedResult:true
        }));
...
async function resumeSmartPendingNode(node, logContext={}){
    ...
    const ownsRecoveredLog = !logContext?.run;
    const effectiveLogContext = logContext?.run ? logContext : restoredSmartPendingLogContext(node);
    const recoveredOutputs = [];
    const logTaskFailure = (message, task) => {
        if(!effectiveLogContext?.run || !message) return;
        const runMs = Math.max(0, nowMs() - Number(effectiveLogContext.runLogStart || nowMs()));
        addSmartGenerationLog({
            run:{...effectiveLogContext.run, taskIds:[task?.taskId].filter(Boolean)},
            outputs:[],
            runMs,
            error:message
        });
    ...
    await Promise.all(tasks.map(async task => {
        ...
        try {
            ...
            if(e && e.imageTaskRecover && e.recoverTaskId){
                ...
                node.pending = Math.max(
                    1,
                    smartPendingTasks(node).reduce((total, item) => total + smartPendingTaskQuantity(item), 0),
                );
    ...
    if(ownsRecoveredLog && effectiveLogContext?.run && recoveredOutputs.length){
        addSmartGenerationLog({
            run:{...effectiveLogContext.run, taskIds:tasks.map(task => task.taskId).filter(Boolean)},
            outputs:recoveredOutputs,
            runMs:Math.max(0, nowMs() - Number(effectiveLogContext.runLogStart || nowMs()))
        });
    }
    if(!smartPendingTasks(node).length) clearSmartPendingLog(node);
