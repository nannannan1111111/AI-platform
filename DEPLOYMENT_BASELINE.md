...
- 基线：已部署 V35，继续在分支 `codex/v35-llm-image2` 上修改，未从其他版本或 worktree 覆盖。
- 功能：修复画布缩略图异步回填早于编辑器监听时事件丢失的问题；网关暂存回填快照，画布初始化后主动消费并重新渲染；原图编辑、下载、保存、删除和最多 4 并发保持不变。
- 数据安全：无新增数据库迁移；不重复上游请求、不重复扣费/退款或额度流水；不删除历史媒体、画布或用户资源。
- 验证：画布媒体回归 `4 passed`；管理端构建、JavaScript 语法和 `git diff --check` 通过。
- 源码标签：`v1.0.16.1`（V36 供应链修订）；镜像：`ghcr.io/nannannan1111111/ai-platform@sha256:02cfe651ce5671320dd8da4e037249412cb8ae4436693b04d4770c5b8cc9cd27`。
...
- 基于已部署 V37，继续在分支 `codex/v35-llm-image2` 上修改，未从其他版本或 worktree 覆盖。
...
- 镜像：`creative-studio:single-host-candidate-v33@sha256:67e3640ed3a4fea16583d9a7632df82ef36892622773ad64f7c6588eda16801b`。
