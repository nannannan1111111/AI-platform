...
- 源码标签 `v1.0.16.1`（V36 供应链修订），生产镜像 `ghcr.io/nannannan1111111/ai-platform@sha256:02cfe651ce5671320dd8da4e037249412cb8ae4436693b04d4770c5b8cc9cd27`。
- 生产已部署：`2026-08-26T08:35:59Z`；V35 摘要 `sha256:75665f6e296ce59555191408b07ef43120164b2cc0f6ce0230878cce0ee9cfd5` 保留回滚。
- 部署后验证：供应链门禁全绿，数据库迁移成功，本机 `/healthz`、`/readyz` 和公网 HTTPS `/readyz` 通过；Web 与 10 个 Worker 使用同一摘要，运行容器数量 11。

## V1.0.17 / V37

- 基于已部署 V36，继续在分支 `codex/v35-llm-image2` 上修改。
- 修复画布缩略图回填被任务恢复流程阻断的问题：缩略图先独立加载，任务恢复或生成结果恢复失败不再让图片停留在透明占位图。
- 缩略图最多 4 路并发、原图编辑/下载、用户媒体归属及保存删除行为保持；无新增数据库迁移。
- 验证：画布媒体回归 `5 passed`；JavaScript 语法和 `git diff --check` 通过。
- 源码标签：`v1.0.17`；生产镜像：`ghcr.io/nannannan1111111/ai-platform@sha256:4561442ff28bcca77818b779f8154dfae4ddf585f095dbb97c704a0f2f50067d`。
